...
- **接入指南**：演示账号、终端命令、自定义客户端认证
- **文本工作台**：摘要、翻译、改写、取消生成、继续调整与保存记录

使用公开 SDK 0.4、清单 v3、通信协议 v4，覆盖 17 类扩展能力。它不是宿主内置功能，不会自动安装或启用。
...
| 模型 | 用途 |
| --- | --- |
| `demo-echo` | 分段回显输入，体验流式返回和取消 |
| `demo-auto` | 经模型路由后调用回显执行器，观察选路、选号与计量 |

本地模型不访问上游，账号资料、额度和计价采用演示数据。需要时，示例操作会准备两个固定演示账号；安装与页面加载不会创建账号。

文本任务使用用户选择的 Key 和模型，可能产生真实用量与费用。网页取文通过宿主网络能力执行，仅接收无凭据的 HTTP(S) 地址，最多读取 256 KiB，不跟随重定向。

插件只处理演示服务商或带 `metadata.capability_workbench: "true"` 标记的请求。大写转换还需 `capability_workbench_uppercase: "true"`，普通文本任务不会被转换为大写。
...
| `POST api/demo-accounts/prepare` | `accounts.ts` | 幂等准备演示账号 |
